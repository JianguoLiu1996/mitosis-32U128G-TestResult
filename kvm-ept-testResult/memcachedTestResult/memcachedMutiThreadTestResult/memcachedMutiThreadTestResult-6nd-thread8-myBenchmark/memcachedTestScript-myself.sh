...
function prepareData(){
	...
	numactl -C 0-5,8-13,16-21,24-29 memtier_benchmark -s $SERVERADDR \
		-P memcache_text \
		--threads=20 \
		--clients=5 \
		--pipeline 32 \
...
function testOne(){
	...
	numactl -C 0-5,8-13,16-21,24-29 memtier_benchmark -s $SERVERADDR \
		...
		--key-maximum=104857600 \
		--key-prefix=memtier- \
		--out-file=${OUTPUTPATH}memcached_test_result_RO_${CONFIG}_${NUMBER}_$(date +"%Y%m%d%H%M%S").log
	wait
	sleep 1m
...
function startRedis(){
	# start memcached
	sudo numactl -C 6-7,14-15,22-23,30-31 memcached -d -m 122880 -p 6379 -u root -t 8
	wait 
...
function startRedisWithPageReplication(){
        sudo numactl -C 6-7,14-15,22-23,30-31 -r 0-3 memcached -d -m 122880 -p 6379 -u root -t 8
	wait 
	ps auxf | grep memcached
	sleep 1s
