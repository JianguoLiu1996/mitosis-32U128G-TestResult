...
function prepareData(){
	...
	numactl -C 0-1,8-9,16-17,24-25 memtier_benchmark -p 6379 \
		-P memcache_text \
		-s $SERVERADDR \
		-t 20 \
		-c 5 \
...
function startRedis(){
	# start memcached
	sudo numactl -C 2-7,10-15,18-23,26-31 memcached -d -m 102400 -p 6379 -u root -t 24
	wait 
...
function startRedisWithPageReplication(){
        sudo numactl -C 2-7,10-15,18-23,26-31 -r 0-3 memcached -d -m 102400 -p 6379 -u root -t 24
	wait 
	ps auxf | grep memcached
	sleep 1s
