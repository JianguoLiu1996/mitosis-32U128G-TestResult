...
NR_PTCACHE_PAGES=131072 # --- 0.5GB per node
...
#     NODE_MAX=`expr $NODE_MAX - 1`
	...
	echo "memcached pid is $REDIS_PID"
        ...
        sudo $CMD_PREFIX keydb-server /etc/keydb/keydb.conf
        REDIS_PID=$(ps aux | grep 'keydb-server' | grep -v grep | grep -v bash | tr -s ' '| cut -d ' ' -f 2)
